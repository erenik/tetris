...
//		return;
	...
	/// Normal-mapping yoooow.
	vec3 normalMapSample = texture2D(normalMap, UV_Coord).xyz;
	vec3 pixelNormal = tangentBitangentNormalMatrix * normalize( texture2D( normalMap, UV_Coord).xyz * 2.0 - 1.0 );
	vec3 normalMapSampleNormalized = normalize(normalMapSample);
	vec3 normal2 = tangentBitangentNormalMatrix * normalMapSampleNormalized;
//	normal = normalMapSample;
	normal = normal2;
	// Normal map looks gooood! :D
//	constantColor.xyz = normalMapSample;
//	constantColor.xyz = normalMapSampleNormalized; //biTangentWorldSpace;
//	constantColor.xyz = normal2;
